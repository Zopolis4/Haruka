...
unsigned int jvideo::in3da (bool vp2)
{
...
void jvideo::out3da (bool vp2, unsigned char data)
{
  int vp;
...
unsigned int jvideo::in3dd()
{
  flag3dd = false;
  return 0;  // FIXME: Return what?
}

void jvideo::out3dd (unsigned char data)
{
  if (!flag3dd)
  {
    flag3dd = true;
...
static void draw1 (unsigned char** p, unsigned char d)
{
  *(*p)++ = d;
}

static void draw2 (unsigned char** p, unsigned char d)
{
  draw1 (p, d);
  draw1 (p, d);
}

static unsigned char* draw_ma (unsigned char* p, int vp, unsigned int ma, unsigned int ra,
                               ...
                               unsigned int mode2, unsigned int blinkcount)
{
  ...
  else  // Graphic mode
  {
    ...
    unsigned int voff;
    if (mode1 & 1)
      voff = (readtop & ~0x7fff) | ((gra << 13) & 0x6000) | ((gma << 1) & 0x1fff);
    else
      voff = (readtop & ~0x3fff) | ((gra << 13) & 0x2000) | ((gma << 1) & 0x1fff);
    unsigned int d1 = readmem.read (voff);
    unsigned int d2 = readmem.read (voff | 1);
    switch (c)
    {
...
void jvideo::convtick (bool disp)
{
...
void jvideo::correct_disp_pos()
{
  ...
  else
  {
    ...
    if ((new_disp_start_x > disp_start_x ? new_disp_start_x - disp_start_x
                                         : disp_start_x - new_disp_start_x) +
            (new_disp_start_y > disp_start_y ? new_disp_start_y - disp_start_y
                                             : disp_start_y - new_disp_start_y) <
        5)
...
void jvideo::convsub (int readtop1, int readtop2, int enable1, int enable2, int si, int len,
                      unsigned char* p, bool disp)
{
  if (disp)
  {
    ...
    if (enable1 && si != 0x1)
      draw_ma (buf1, 0, crtc.get_ma(), crtc.get_ra(), gma1, gra1, crtc.get_cursor (true), program,
               kanjirom, readtop1, mode1[0], mode2[0], blinkcount);
    if (enable2 && si != 0x0)
      draw_ma (buf2, 1, crtc.get_ma(), crtc.get_ra(), gma2, gra2, crtc.get_cursor (true), vram,
               kanjirom, readtop2, mode1[1], mode2[1], blinkcount);
    if (si & 0xe)
    {
      if (!enable1)
        memset (&buf1[0], 0, len);
...
void jvideo::ex_convsub (int enable, int len, unsigned char* p, bool disp)
{
  if (disp)
  {
    if (!enable)
    ...
    if (!(ex_reg2 & 0x2))  // Text mode
    {
      ...
      {
        color = ((ex_reg5 & 0x40) ? 8 : 0) | ((d2 & 0x8) ? 0 : 4) | ((d2 & 0x40) ? 0 : 2) |
                ((d2 & 0x80) ? 0 : 1);
        if (d2 & 0x4)  // RV
        {
          bg = color;
    ...
    {
      unsigned int voff = (((gra3 << 15) & 0x8000) ^ 0x8000) | ((gma3 << 1) & 0x7fff);
      unsigned int d1 = vram.read (voff);
      unsigned int d2 = vram.read (voff | 1);
...
void jvideo::ex_conv (int clockcount)
{
  ...
  while (clockcount > 0)
  {
    ...
    while (nchars-- > 0)
    {
      ...
      {
        ex_convsub (enable, len, videohw.get_pointer (draw_x, draw_y), disp);
...
void jvideo::conv (int clockcount)
{
  ...
  while (convcount >= len)
  {
    ...
    if (draw_y >= 0 && draw_y < SURFACE_HEIGHT && draw_x >= 0 && draw_x + len <= SURFACE_WIDTH)
    {
...
void jvideo::floppyaccess (int n) {}

jvideo::jvideo (hw& videohw, jmem& program_arg, jmem& kanjirom_arg)
    : program (program_arg), vram (65536), kanjirom (kanjirom_arg), videohw (videohw)
{
  int i;
...
unsigned int jvideo::j46505::inb (unsigned int addr)
{
  if ((addr & 1) != 0)
...
void jvideo::j46505::outb (unsigned int addr, unsigned int val)
{
  if ((addr & 1) != 0)
...
unsigned int jvideo::j46505::get_reg (unsigned int idx)
{
  switch (idx)
  {
  case SADDRHIGH:
...
void jvideo::j46505::set_reg (unsigned int idx, unsigned int val)
{
  switch (idx)
  {
  case HTOTAL:
...
unsigned int jvideo::j46505::get_ma()
{
  return iy * reg[HDISP] + ix + (reg[SADDRLOW] | reg[SADDRHIGH] << 8);
}

unsigned int jvideo::j46505::get_ra()
{
  return ira;
}

bool jvideo::j46505::get_cursor (bool force_blink)
{
  if (!get_disp())
    return false;
...
static unsigned int get_vsync_lines (unsigned int hsyncwidth)
{
...
bool jvideo::j46505::get_vsync()
{
  if (iy >= reg[VSYNCPOS] &&
      (iy - reg[VSYNCPOS]) * (reg[MAXSCANLINE] + 1 + (reg[INTERMODE] & 1)) + ira <
...
void jvideo::out3d4 (unsigned char data)
{
  crtc.outb (0, data);
}

unsigned char jvideo::in3d5()
{
  return crtc.inb (1);
}

void jvideo::out3d5 (unsigned char data)
{
  crtc.outb (1, data);
}
